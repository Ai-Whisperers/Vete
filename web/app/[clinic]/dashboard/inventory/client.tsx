...
import {
    ...
    ArrowUp, ArrowDown, MoreHorizontal, Eye, Copy, Clock
} from 'lucide-react';
...
export default function InventoryClient({ googleSheetUrl }: InventoryClientProps) {
    ...
    const fetchProducts = useCallback(async (query = '', page = 1, limit = pagination.limit) => {
        ...
        try {
            let url = `/api/store/products?clinic=${clinic}&search=${encodeURIComponent(query)}&page=${page}&limit=${limit}`;
    ...
    }, [clinic, selectedCategory, stockFilter, pagination.limit]);
                                        ...
                                        return (
                                            <tr key={p.id} className="group hover:bg-gray-50/50 transition-colors">
                                                ...
                                                <td className="px-4 py-3 text-right">
                                                    <span className="font-bold text-gray-900">{formatPrice(p.base_price)}</span>
                                                </td>
