...
function SubmitButton() {
  return (
    <button 
      type="submit" 
      className="bg-[var(--primary)] text-white font-bold py-3 px-8 rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all flex items-center gap-2"
    >
      ...
      <div className="bg-white rounded-3xl shadow-lg border border-gray-100 p-8">
          ...
          <form action={updateProfile} className="space-y-6">
              ...
              <div className="flex items-center gap-6 pb-6 border-b border-gray-100">
                  <div className="w-24 h-24 rounded-full bg-gray-100 flex items-center justify-center text-gray-300 font-bold text-3xl">
                      {profile?.avatar_url ? (
                          <img src={profile.avatar_url} className="w-full h-full rounded-full object-cover" />
                      ) : (
                          profile?.full_name?.[0] || <Icons.User className="w-10 h-10" />
                      )}
                  </div>
                  <div>
                      <h3 className="font-bold text-lg text-gray-800">{profile?.email}</h3>
