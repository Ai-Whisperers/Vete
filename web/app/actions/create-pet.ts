...
const createPetSchema = z.object({
  name: z
    ...
    .regex(/^[a-zA-ZáéíóúüñÁÉÍÓÚÜÑ\s'-]+$/, "El nombre solo puede contener letras, espacios, apóstrofes y guiones"),
...
export async function createPet(prevState: ActionResult | null, formData: FormData): Promise<ActionResult> {
  ...
  if (authError || !user) {
    return {
      success: false,
      error: "Debes iniciar sesión para registrar una mascota. Por favor, inicia sesión y vuelve a intentarlo."
    };
  }
  ...
  if (!validation.success) {
    ...
    return {
      success: false,
      error: "Por favor, revisa los campos marcados en rojo y corrige los errores antes de continuar.",
      fieldErrors
    };
  }
    ...
    if (insertError.code === "23505") {
      ...
      if (insertError.message.includes("microchip")) {
        return {
          ...
          fieldErrors: {
            microchip_id: "Este microchip ya está registrado en el sistema. Verifica el número o contacta a la clínica si crees que es un error."
          }
        };
      }
      ...
      userMessage += "No tienes permiso para registrar mascotas en esta clínica. Contacta a la clínica para más información.";
    } else {
      userMessage += "Por favor, intenta de nuevo en unos minutos. Si el problema persiste, contacta a soporte.";
    }
