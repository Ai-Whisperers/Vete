...
import { withActionAuth } from '@/lib/actions'
 ...
 */
export const getTimeOffTypes = withActionAuth(
  async (
    { profile, supabase },
    clinicSlug: string
  ): Promise<{
 ...
 */
export const getTimeOffRequests = withActionAuth(
  async (
    { user, profile, supabase, isStaff },
    clinicSlug: string,
    filters?: {
 ...
 */
export const createTimeOffRequest = withActionAuth(
  async (
    { user, supabase },
    clinicSlug: string,
 ...
 * Admin only
 */
export const updateTimeOffRequestStatus = withActionAuth(
  async (
    ...
    // Check admin permission (already verified by requireAdmin, but also check tenant)
 ...
 */
export const cancelTimeOffRequest = withActionAuth(
  async (
    { user, profile, supabase, isAdmin },
    requestId: string
  ): Promise<CalendarActionResult> => {
    ...
    // Check permission - must be the requester or admin of same tenant
    const isRequester = staffProfile.user_id === user.id

    if (!isRequester) {
      if (!isAdmin || profile.tenant_id !== request.tenant_id) {
 ...
 */
export const getTimeOffBalances = withActionAuth(
  async (
    { user, profile, supabase, isStaff },
    clinicSlug: string,
    staffProfileId?: string
 ...
 * For admin dashboard
 */
export const getPendingTimeOffCount = withActionAuth(
  async (
    { profile, supabase },
    clinicSlug: string
  ): Promise<{
    ...
    // Admin permission already verified by requireAdmin
    // Just check tenant match
    if (profile.tenant_id !== clinicSlug) {
      return { count: 0 }
    }
