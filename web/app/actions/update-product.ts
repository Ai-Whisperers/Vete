...
export async function updateProduct(prevState: ActionResult | null, formData: FormData): Promise<ActionResult> {
  ...
  if (authError || !user) {
    return {
      success: false,
      error: "Debes iniciar sesión para actualizar productos."
    };
  }
  ...
  if (!profile || !['vet', 'admin'].includes(profile.role)) {
    return {
      success: false,
      error: "Solo veterinarios y administradores pueden actualizar productos."
    };
  }
  ...
  if (updateError) {
    ...
    return {
      success: false,
      error: "No se pudo actualizar el producto. Por favor, intenta de nuevo."
    };
  }
