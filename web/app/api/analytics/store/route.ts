...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const [todayOrders, weekOrders, monthOrders, yearOrders] = await Promise.all([
      supabase.from('store_orders').select('total').eq('tenant_id', tenantId)
        .gte('created_at', todayStart.toISOString()).in('status', validStatuses).is('deleted_at', null),
      supabase.from('store_orders').select('total').eq('tenant_id', tenantId)
        .gte('created_at', weekStart.toISOString()).in('status', validStatuses).is('deleted_at', null),
      supabase.from('store_orders').select('total').eq('tenant_id', tenantId)
        .gte('created_at', monthStart.toISOString()).in('status', validStatuses).is('deleted_at', null),
      supabase.from('store_orders').select('total').eq('tenant_id', tenantId)
        .gte('created_at', yearStart.toISOString()).in('status', validStatuses).is('deleted_at', null),
    ]);
    ...
    interface OrderRow { status: string; total: number }
    ...
    interface TrendOrder { created_at: string; total: number }
    ...
    const [couponsResult, couponOrdersResult] = await Promise.all([
      ...
      supabase.from('store_orders').select('discount_amount, coupon_id').eq('tenant_id', tenantId).is('deleted_at', null)
    ]);

    interface CouponRow { is_active: boolean; used_count: number }
    interface CouponOrderRow { discount_amount: number; coupon_id: string | null }
