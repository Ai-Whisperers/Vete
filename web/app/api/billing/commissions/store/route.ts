/**
 * Store Commissions API (under Billing namespace) - Clinic View
 *
 * GET /api/billing/commissions/store - List clinic's store commissions
 *
 * This endpoint mirrors /api/store/commissions but is grouped under billing
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    logger.error('Error fetching store commissions', {
      tenantId,
      userId: user.id,
      error: e instanceof Error ? e.message : 'Unknown',
    })
    return apiError('DATABASE_ERROR', HTTP_STATUS.INTERNAL_SERVER_ERROR, {
      details: { message: 'Error al cargar comisiones de tienda' },
    })
  }
}
