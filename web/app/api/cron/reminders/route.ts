    ...
    for (const reminder of reminders as Reminder[]) {
      ...
      try {
        ...
        const pet = reminder.pet
          ? (Array.isArray(reminder.pet) ? reminder.pet[0] : reminder.pet)
          : null;
      ...
      case 'appointment': {
        ...
        if (data) {
          ...
          return {
            ...
            appointment_time: date.toLocaleTimeString('es-PY', { hour: '2-digit', minute: '2-digit' }),
      ...
      case 'invoice': {
        ...
        if (data) {
          return {
            ...
            amount: data.total
              ? `Gs. ${data.total.toLocaleString('es-PY')}`
              : '',
            due_date: data.due_date
              ? new Date(data.due_date).toLocaleDateString('es-PY')
              : '',
          };
        }
        break;
