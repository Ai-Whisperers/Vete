...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (parseOnly) {
      ...
      const rows = rawData.slice(1).filter(row => row.some(cell => cell !== null && cell !== undefined && String(cell).trim() !== ''));
...
async function processPreview(
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const price = Number.parseFloat(String(
      ...
      const quantity = Number.parseFloat(String(
      ...
      const cost = Number.parseFloat(String(
      ...
      const isNewProduct = ['new product', 'new', 'nuevo', 'nuevo producto', 'crear'].includes(normalizedOp);
      const isBuy = ['buy', 'purchase', 'compra', 'comprar'].includes(normalizedOp);
      const isAdjustment = ['adj', 'adjustment', 'ajuste', 'ajustar', 'adjust'].includes(normalizedOp);
      ...
      if (isNewProduct) {
        if (!name) {
          ...
          summary.errors++;
        } else if (category && !categoryMap.has(category.toLowerCase()) && !categorySlugMap.has(category.toLowerCase())) {
          previewRow.status = 'error';
