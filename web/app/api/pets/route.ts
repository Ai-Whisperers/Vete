...
export async function POST(request: Request) {
  ...
  try {
    ...
    logger.error('Error in POST /api/pets', { error: error instanceof Error ? error.message : String(error) });
...
export async function GET(request: Request) {
  ...
  if (error) {
    logger.error('Error fetching pets', { userId, error: error instanceof Error ? error.message : String(error) });
    return apiError('DATABASE_ERROR', HTTP_STATUS.INTERNAL_SERVER_ERROR);
  }
