...
import type { ClinicTheme, ColorScale, StatusColorSet, NeutralColors } from '@/lib/types/clinic-config';
...
export function ClinicThemeProvider({ theme }: { theme: ClinicTheme }): null {
  useEffect(() => {
    ...
    if (neutral) {
      const keys: (keyof NeutralColors)[] = ['50', '100', '200', '300', '400', '500', '600', '700', '800', '900', '950'];
      keys.forEach(key => {
    ...
    if (accents) {
      const accentNames = ['teal', 'purple', 'pink', 'orange', 'cyan', 'lime', 'amber', 'rose', 'indigo', 'emerald'] as const;
