...
export function TimelinePanel({ hospitalization }: TimelinePanelProps): JSX.Element {
  ...
  const events: TimelineEvent[] = [
    ...(hospitalization.vitals || []).map(v => ({ type: 'vital' as const, date: v.recorded_at, data: v })),
    ...(hospitalization.treatments || []).filter(t => t.administered_at).map(t => ({ type: 'treatment' as const, date: t.administered_at!, data: t })),
    ...(hospitalization.feedings || []).map(f => ({ type: 'feeding' as const, date: f.feeding_time, data: f })),
    ...(hospitalization.transfers || []).map(t => ({ type: 'transfer' as const, date: t.transfer_date, data: t })),
    ...(hospitalization.visits || []).map(v => ({ type: 'visit' as const, date: v.visit_start, data: v })),
