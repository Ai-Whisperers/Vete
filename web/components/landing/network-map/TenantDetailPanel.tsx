 ...
 *
 * Overlay panel showing detailed tenant information.
 */
...
export function TenantDetailPanel({ tenant, onClose }: TenantDetailPanelProps) {
  return (
    <div className="bg-[var(--bg-dark)]/95 absolute inset-0 z-20 overflow-y-auto p-4 backdrop-blur-sm">
      ...
      <div className="pt-8">
        <h3 className="mb-2 text-2xl font-bold text-white">{tenant.name}</h3>
        ...
        <div className="mb-6 flex items-center gap-2">
          {[...Array(5)].map((_, i) => (
            <Star
              key={i}
              className={`h-5 w-5 ${
                i < Math.floor(tenant.rating)
                  ? 'fill-[var(--primary)] text-[var(--primary)]'
                  : 'text-white/20'
              }`}
            />
          ))}
          <span className="font-bold text-white">{tenant.rating}</span>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="space-y-1 text-sm">
            <div className="flex justify-between">
              ...
              <span className="text-white">{tenant.hours.weekdays}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-white/50">Sabado</span>
              <span className="text-white">{tenant.hours.saturday}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-white/50">Domingo</span>
              <span className="text-white">{tenant.hours.sunday}</span>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {tenant.specialties.map((spec, idx) => {
        ...
        <div className="space-y-3">
          ...
          <a
            href={`https://www.google.com/maps/dir/?api=1&destination=${tenant.coordinates.lat},${tenant.coordinates.lng}`}
            target="_blank"
