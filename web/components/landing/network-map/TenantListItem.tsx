 ...
 *
 * Individual tenant card in the list view.
 */
...
export function TenantListItem({ tenant, isSelected, onClick }: TenantListItemProps) {
  ...
  let isOpen = false
  if (isWeekday) {
    const [open, close] = tenant.hours.weekdays.split(' - ').map((t) => parseInt(t.split(':')[0]))
    isOpen = hour >= open && hour < close
  } else if (isSaturday) {
    const [open, close] = tenant.hours.saturday.split(' - ').map((t) => parseInt(t.split(':')[0]))
    isOpen = hour >= open && hour < close
  } else if (tenant.emergency24h) {
    isOpen = true
  }
  ...
  return (
    ...
    >
      <div className="mb-2 flex items-start justify-between">
        <div>
          ...
          <p className="flex items-center gap-1 text-sm text-white/50">
            ...
            {tenant.neighborhood}, {tenant.city}
          </p>
        </div>
        <div className="flex flex-col items-end gap-1">
          <div className="flex items-center gap-1">
            <Star className="h-3 w-3 fill-[var(--primary)] text-[var(--primary)]" />
            <span className="text-sm font-bold text-white">{tenant.rating}</span>
          </div>
          <span
      ...
      <div className="mb-3 flex flex-wrap gap-1">
        {tenant.specialties.slice(0, 3).map((spec, idx) => (
      ...
      <div className="flex items-center justify-between text-xs">
        <span className="text-white/40">
          ...
          {isWeekday ? tenant.hours.weekdays : isSaturday ? tenant.hours.saturday : tenant.hours.sunday}
        </span>
        {tenant.emergency24h && (
