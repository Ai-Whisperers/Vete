...
import { Check, MessageCircle, Sparkles, Gift } from 'lucide-react'
import {
  pricingTiers,
  discounts,
  trialConfig,
...
function PricingCard({ tier, features, cta, billingPeriod }: PricingCardProps): React.ReactElement {
  ...
  const showSavings = billingPeriod === 'yearly' && !isFree
    ...
    >
      ...
      <div className="mb-6">
        <div className="flex items-baseline gap-1">
          ...
          {!isFree && <span className="text-[var(--landing-text-muted)]">/mes</span>}
        </div>
...
export function PricingSection(): React.ReactElement {
  ...
  // With only 2 tiers, no need for enterprise handling
  const profesionalTier = pricingTiers.find((t) => t.monthlyPrice > 0)

  // Calculate savings for yearly on the Professional tier
  const yearlySavings = profesionalTier
    ? getPricingDetails(profesionalTier, 'yearly').savings
    : 0
      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="mb-12 text-center md:mb-16">
          <h2 className="mb-4 text-3xl font-black text-[var(--landing-text-primary)] md:text-4xl lg:text-5xl">
            Solo 2 planes. <span className="text-3xl md:text-4xl lg:text-5xl text-[var(--landing-primary)]">Elige el tuyo.</span>
          </h2>
          <p className="mx-auto max-w-xl text-lg text-[var(--landing-text-secondary)]">
            Sin tarjeta de credito. Sin compromisos. Sin letra chica.
          </p>
          ...
          <div className="mt-8 flex flex-col items-center gap-3">
            ...
            {billingPeriod === 'yearly' && yearlySavings > 0 && (
              <p className="text-sm font-medium text-green-600">
                Con pago anual ahorras Gs {yearlySavings.toLocaleString('es-PY')} por ano
              </p>
            )}
          </div>
        ...
        <div className="mt-8 text-center">
          <p className="text-sm text-[var(--landing-text-muted)]">
            Comision del 3% solo en ventas de la tienda online.
          </p>
        </div>
      </div>
    </section>
