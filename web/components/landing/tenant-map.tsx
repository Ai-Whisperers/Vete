...
export function TenantMap({ clinics }: ClinicMapProps) {
  const [isMounted, setIsMounted] = useState(false)
  const [MapComponents, setMapComponents] = useState<LeafletComponents | null>(null)
  const [userLocation, setUserLocation] = useState<[number, number] | null>(null)
  const [locationStatus, setLocationStatus] = useState<'idle' | 'loading' | 'success' | 'error'>('idle')
  const [sortedClinics, setSortedClinics] = useState<TenantLocation[]>(clinics)
  ...
  function MapController() {
    ...
    useEffect(() => {
      if (userLocation && sortedClinics.length > 0) {
        ...
        clinicsToShow.forEach((clinic: TenantLocation) => {
