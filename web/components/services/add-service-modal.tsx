  ...
  return (
    ...
    >
      <div className="space-y-6">
        {/* Service Info */}
        <div className="flex items-start gap-4 p-4 bg-[var(--bg-subtle)] rounded-xl">
          {service.image && (
            <img
              src={service.image}
              alt={service.title}
        ...
        {/* Price Calculation */}
        {selectedPet && (
          <div className="p-4 bg-gradient-to-r from-[var(--primary)]/5 to-[var(--primary)]/10 rounded-xl border border-[var(--primary)]/20">
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${getSizeBadgeColor(selectedPet.size_category)}`}>
            ...
            {variantHasSizePricing && (
              <>
                <div className="flex items-center justify-between mb-2 text-sm">
                  ...
                  <span className="text-[var(--text-secondary)]">{formatPriceGs(variant.price_value)}</span>
                </div>
                {priceDifference !== 0 && (
                  <div className="flex items-center justify-between mb-2 text-sm">
                    <span className="text-[var(--text-muted)]">Ajuste por tamaño:</span>
                    <span className={priceDifference > 0 ? "text-amber-600" : "text-green-600"}>
                      {priceDifference > 0 ? "+" : ""}{formatPriceGs(priceDifference)}
                    </span>
                  </div>
                )}
                <div className="h-px bg-[var(--primary)]/20 my-3" />
              </>
            )}
        ...
        <ModalFooter>
          <button
            ...
            onClick={handleClose}
            disabled={isAdding}
            className="px-4 py-2 text-[var(--text-secondary)] font-medium hover:bg-[var(--bg-subtle)] rounded-xl transition-colors disabled:opacity-50"
          >
            Cancelar
          </button>
          <button
            ...
            className={`px-6 py-2 font-bold rounded-xl transition-all flex items-center gap-2 ${
              justAdded
                ...
                : "bg-[var(--primary)] text-white hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed"
            }`}
          >
            {isAdding ? (
              <>
