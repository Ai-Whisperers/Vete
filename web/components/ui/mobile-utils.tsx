  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {isRefreshing ? (
            <Icons.Loader2 className="h-5 w-5 text-[var(--primary)]" />
          ) : (
            <Icons.ArrowDown className="h-5 w-5 text-[var(--primary)]" />
          )}
        </div>
      </div>
  ...
  const content = (
    ...
    >
      ...
      {(onClick || href) && !trailing && (
        <Icons.ChevronRight className="h-5 w-5 shrink-0 text-gray-400" />
      )}
    </div>
  )
