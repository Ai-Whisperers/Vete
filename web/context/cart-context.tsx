...
interface CartContextType {
  ...
  getStockStatus: (itemId: string) => { atLimit: boolean; nearLimit: boolean; available: number } | null;
}
...
export function CartProvider({ children }: { readonly children: React.ReactNode }) {
  ...
  const addItem = useCallback((newItem: Omit<CartItem, "quantity">, quantity: number = 1): StockValidationResult => {
  ...
  const updateQuantity = useCallback((id: string, delta: number): StockValidationResult => {
    ...
    setItems((prev) => {
      ...
      // Stock validation for products
      if (existing.type === "product" && existing.stock !== undefined && newQuantity > existing.stock) {
  ...
  const getStockStatus = useCallback((itemId: string): { atLimit: boolean; nearLimit: boolean; available: number } | null => {
  ...
  const contextValue = useMemo(() => ({
    ...
    syncStatus, syncError, isLoggedIn, syncToDatabase, loadFromDatabase,
    getStockStatus
  }), [items, total, itemCount, discount, syncStatus, syncError, isLoggedIn,
      addItem, updateQuantity, removeItem, clearCart, syncToDatabase, loadFromDatabase, getStockStatus]);
