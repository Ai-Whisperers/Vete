...
const SCHEMA_COLUMNS = {
  ...
  drug_dosages: ['name', 'species', 'dose_mg_per_kg', 'dose_range_min', 'dose_range_max', 'route', 'frequency', 'notes', 'contraindications', 'category'],
  growth_standards: ['species', 'breed', 'breed_category', 'gender', 'age_weeks', 'weight_kg', 'percentile'],
  lab_test_catalog: ['code', 'name', 'category', 'description', 'sample_type', 'unit', 'reference_min', 'reference_max', 'turnaround_hours', 'price'],
  insurance_providers: ['code', 'name', 'contact_email', 'contact_phone', 'website', 'claim_process', 'notes'],
  services: ['name', 'description', 'category', 'base_price', 'duration_minutes', 'is_active', 'requires_appointment', 'currency'],
  ...
  store_brands: ['slug', 'name', 'logo_url', 'website', 'description', 'country_origin', 'is_active', 'is_global_catalog'],
  store_categories: ['slug', 'name', 'description', 'parent_id', 'level', 'display_order', 'is_active', 'is_global_catalog'],
  suppliers: ['name', 'legal_name', 'tax_id', 'contact_info', 'website', 'supplier_type', 'payment_terms', 'delivery_time_days', 'is_active'],
  store_products: ['sku', 'name', 'description', 'brand_id', 'category_id', 'base_price', 'purchase_unit', 'sale_unit', 'weight_grams', 'requires_prescription', 'is_active', 'barcode', 'image_url', 'is_global_catalog'],
};
...
async function seedReferenceData(verbose: boolean = false): Promise<void> {
  ...
  if (diagnosisData?.diagnosis_codes) {
    const { valid } = validateArray(diagnosisData.diagnosis_codes, DiagnosisCodeSchema, 'Diagnosis codes');
  ...
  if (growthData?.growth_standards) {
    const { valid } = validateArray(growthData.growth_standards, GrowthStandardSchema, 'Growth standards');
  ...
  if (insuranceData?.insurance_providers) {
    const { valid } = validateArray(insuranceData.insurance_providers, InsuranceProviderSchema, 'Insurance providers');
...
async function seedTenant(tenantId: string): Promise<void> {
  ...
  const tenantsData = loadJSON<{ tenants: any[] }>(
    join(DATA_DIR, '00-core', 'tenants.json')
  );
...
async function seedClinicData(tenantId: string, verbose: boolean = false): Promise<void> {
  ...
  if (paymentData?.payment_methods) {
    const { valid } = validateArray(paymentData.payment_methods, PaymentMethodSchema, 'Payment methods');
  ...
  if (kennelsData?.kennels) {
    ...
    const { error } = await client
      .from('kennels')
      .upsert(kennels, { onConflict: 'tenant_id,code' });
    if (error) {
...
async function seedStoreData(tenants: string[], verbose: boolean = false): Promise<void> {
  ...
  // Brands (global)
  const brandsData = loadJSON<{ brands: any[] }>(
    join(DATA_DIR, '03-store', 'brands.json')
  );
  if (brandsData?.brands) {
    ...
    const { error } = await client
      .from('store_brands')
      .upsert(filtered, { onConflict: 'slug' });
    if (error) {
  ...
  const suppliersData = loadJSON<{ suppliers: any[] }>(
    join(DATA_DIR, '03-store', 'suppliers.json')
  );
...
async function seedProducts(verbose: boolean = false): Promise<void> {
  ...
  console.log(`  ${statusIcon} Products: ${totalInserted} total from ${files.length} files${totalValidationErrors > 0 ? ` (${totalValidationErrors} validation errors)` : ''}`);
}
  ...
  if (kennels && kennels.length > 0 && allPets && allPets.length > 0 && vetId) {
    for (let i = 0; i < Math.min(3, kennels.length, allPets.length); i++) {
      ...
      if (!error) {
        ...
        const vitals = {
          hospitalization_id: (await client.from('hospitalizations').select('id').eq('pet_id', pet.id).order('admitted_at', { ascending: false }).limit(1).single()).data?.id,
  ...
  if (labTests && labTests.length > 0 && allPets && allPets.length > 0 && vetId) {
    for (let i = 0; i < Math.min(5, allPets.length); i++) {
      ...
      const labOrder = {
        ...
        notes: ['Chequeo anual', 'Pre-quirúrgico', 'Control de tratamiento', 'Diagnóstico inicial', 'Urgencia'][i % 5],
      };

      const { data: insertedOrder, error } = await client.from('lab_orders').insert(labOrder).select('id').single();
...
async function verifySeedResults(config: SeedConfig): Promise<void> {
  ...
  if (config.type !== 'basic') {
    const { count: diagnosisCount } = await client.from('diagnosis_codes').select('*', { count: 'exact', head: true });
    results.push({
    ...
    const { count: growthCount } = await client.from('growth_standards').select('*', { count: 'exact', head: true });
    results.push({
  ...
  for (const tenantId of config.tenants) {
    const { count: tenantCount } = await client.from('tenants').select('*', { count: 'exact', head: true }).eq('id', tenantId);
    results.push({
    ...
    const { count: servicesCount } = await client.from('services').select('*', { count: 'exact', head: true }).eq('tenant_id', tenantId);
    results.push({
    ...
    const { count: paymentMethodsCount } = await client.from('payment_methods').select('*', { count: 'exact', head: true }).eq('tenant_id', tenantId);
    results.push({
    ...
    if (config.type === 'full' || config.type === 'demo') {
      const { count: productsCount } = await client.from('store_products').select('*', { count: 'exact', head: true });
      results.push({
      ...
      const { count: brandsCount } = await client.from('store_brands').select('*', { count: 'exact', head: true });
      results.push({
        table: 'store_brands',
        expected: '>0',
      ...
      const { count: inventoryCount } = await client.from('store_inventory').select('*', { count: 'exact', head: true }).eq('tenant_id', tenantId);
      results.push({
    ...
    // Demo type checks
    if (config.type === 'demo') {
      const { count: ownersCount } = await client.from('profiles').select('*', { count: 'exact', head: true }).eq('tenant_id', tenantId).eq('role', 'owner');
      results.push({
      ...
      const { count: petsCount } = await client.from('pets').select('*', { count: 'exact', head: true }).eq('tenant_id', tenantId);
      results.push({
        table: `pets(${tenantId})`,
        expected: '>0',
        actual: petsCount || 0,
      ...
      const { count: vaccinesCount } = await client.from('vaccines').select('*', { count: 'exact', head: true });
      results.push({
        table: 'vaccines',
        expected: '>0',
