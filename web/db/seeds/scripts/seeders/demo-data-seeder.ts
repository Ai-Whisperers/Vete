...
export class DemoDataSeeder {
    ...
    for (const pet of pets) {
      ...
      for (let i = 0; i < numRecords; i++) {
        ...
        records.push({
          ...
          heart_rate_bpm: isCat ? 140 + Math.floor(Math.random() * 40) : 70 + Math.floor(Math.random() * 40),
          respiratory_rate_rpm: isCat ? 20 + Math.floor(Math.random() * 15) : 15 + Math.floor(Math.random() * 10),
    ...
    for (const pet of pets) {
      ...
      for (let i = 0; i < Math.min(numVaccines, vaccineList.length); i++) {
        ...
        vaccines.push({
          pet_id: pet.id,
          name: vaccineList[i],
          administered_date: new Date(Date.now() - daysAgo * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
          next_due_date: new Date(Date.now() + daysUntilDue * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
          status,
  ...
  private async seedPrescriptions(
    ...
    for (const pet of petsToUse) {
      ...
      prescriptions.push({
        ...
        prescribed_date: new Date(Date.now() - daysAgo * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        valid_until: new Date(Date.now() - daysAgo * 24 * 60 * 60 * 1000 + validDays * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
  ...
  private async seedLabOrders(
    ...
    for (const pet of petsToUse) {
      ...
      orders.push({
        ...
        status,
        lab_type: 'in_house',
        collected_at: status !== 'pending' ? new Date(Date.now() - daysAgo * 24 * 60 * 60 * 1000).toISOString() : null,
        collected_by: status !== 'pending' ? this.vetId : null,
        completed_at: status === 'completed' ? new Date(Date.now() - (daysAgo - 1) * 24 * 60 * 60 * 1000).toISOString() : null,
      });
    }
  ...
  private async seedAppointments(
    ...
    const notes = [
      'Consulta general', 'Control de vacunas', 'Chequeo anual', 'Seguimiento tratamiento',
      'Limpieza dental', 'Control de peso', 'Consulta dermatológica', 'Emergencia',
    ];
  ...
  private async seedHospitalizations(
    ...
    for (let i = 0; i < petsToUse.length; i++) {
      ...
      hospitalizations.push({
        ...
        expected_discharge: new Date(Date.now() + (3 - daysAgo) * 24 * 60 * 60 * 1000).toISOString(),
        reason: r.reason,
  ...
  private async seedInvoices(
    ...
    for (const owner of owners.slice(0, 8)) {
      ...
      invoices.push({
        ...
        invoice_date: new Date(Date.now() - daysAgo * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        due_date: new Date(Date.now() - (daysAgo - 30) * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        subtotal,
        tax_amount: tax,
        total: subtotal + tax,
        status,
