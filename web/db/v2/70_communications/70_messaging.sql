...
ON public.message_templates (code) WHERE tenant_id IS NULL;
...
-- Conversations: Staff see all, clients see own
...
-- Messages
CREATE POLICY "Access messages via conversation" ON public.messages
    FOR ALL TO authenticated
    USING (
        EXISTS (
...
-- Notification queue: Staff only
CREATE POLICY "Staff manage queue" ON public.notification_queue
    FOR ALL TO authenticated
    USING (public.is_staff_of(tenant_id));

-- Communication preferences: Users manage own, staff view
