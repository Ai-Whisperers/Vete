...
export async function loginAs(
  ...
  try {
    // Navigate to login page with longer timeout for slow dev server
    await page.goto(`/${clinic}/portal/login`, { timeout: 30000 });

    // Wait for login form to be visible
    await page.waitForSelector('form', { state: 'visible', timeout: 10000 });

    // Fill in credentials - use ID selector to avoid newsletter email field
    const emailInput = page.locator('#email'); // Login form email (not newsletter)
    ...
    // Submit form - click "Sign In" button (avoid Google OAuth and newsletter)
    const submitButton = page.getByRole('button', { name: /sign in/i });
    await submitButton.click();

    // Wait for navigation to complete
    if (waitForRedirect) {
      console.log(`[E2E/Auth] Waiting for login redirect...`);
      
      // Wait for either URL change OR content change (Supabase Auth uses client-side navigation)
      try {
        // First, try to wait for URL to change away from /login
        await page.waitForURL((url) => !url.pathname.includes('/login'), {
          timeout: 10000,
        });
        console.log(`[E2E/Auth] URL changed to: ${page.url()}`);
      } catch (urlError) {
        // URL didn't change - might be client-side routing, check for content change instead
        console.log(`[E2E/Auth] URL didn't change (${page.url()}), checking for content change...`);
      }

      // Verify successful login by checking for portal/dashboard content
      const successIndicators = [
        page.getByRole('heading', { name: /good (morning|afternoon|evening)/i }), // Portal greeting
        page.getByRole('link', { name: /book appointment|add pet|mis mascotas/i }), // Portal actions
      ...
      for (const indicator of successIndicators) {
        try {
          ...
          foundIndicator = true;
          console.log(`[E2E/Auth] Login success - found indicator`);
          break;
        } catch {
          // Try next indicator
        }
      ...
      if (!foundIndicator) {
        console.log(`[E2E/Auth] No success indicators found at URL: ${page.url()}`);
