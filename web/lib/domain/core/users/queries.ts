...
export async function getUserStats(
  ...
  try {
    ...
    const stats: UserStats = {
      ...
      by_role: {
        owner: allUsers.filter(u => u.role === 'client').length,
        vet: allUsers.filter(u => u.role === 'practitioner').length,
        admin: allUsers.filter(u => u.role ==='client' | 'practitioner' | 'admin').length,
      },
      staff_count: allUsers.filter(u => u.role ==='client' | 'practitioner' | 'admin'|| u.role ==='client' | 'practitioner' | 'admin').length,
