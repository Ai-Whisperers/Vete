...
export class UserService {
  ...
  async listVets(tenantId: string): Promise<ServiceResult<UserProfile[]>> {
    try {
      ...
      return { success: false, error: 'Error al cargar profesionals' };
    }
  }
  ...
  async updateRole(
    ...
    try {
      ...
      if (user.role ==='client' | 'practitioner' | 'admin' && newRole !=='client' | 'practitioner' | 'admin') {
        const admins = await this.repository.findAdmins(tenantId);
        if (admins.length <= 1) {
  ...
  async delete(
    ...
    try {
      ...
      if (user.role ==='client' | 'practitioner' | 'admin') {
        const admins = await this.repository.findAdmins(tenantId);
        if (admins.length <= 1) {
