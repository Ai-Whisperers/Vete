...
export function loadProducts(): FlatProduct[] {
  ...
  const files = readdirSync(PRODUCTS_DIR).filter(f => f.startsWith('products-') && f.endsWith('.json'));
 ...
 */
export function getDataSummary(): { categories: number; brands: number; suppliers: number; products: number } {
  return {
