...
function getGuideContent(): string[][] {
  return [
    ...
    ['', '', '', '', '', '', '', ''],
    ['• Agregar más filas de datos', '', '• Eliminar filas de encabezado (fila 1)', '', '• Usa los menús desplegables (▼)', '', '', ''],
    ['• Modificar datos de ejemplo', '', '• Cambiar nombres de las hojas', '', '• Precios siempre en Guaraníes', '', '', ''],
    ['• Borrar datos de ejemplo', '', '• Renombrar columnas', '', '• Fechas en formato dd/mm/yyyy', '', '', ''],
    ['• Usar tus propios códigos', '', '• Editar la plantilla original', '', '• Usa filtros para buscar', '', '', ''],
    ['• Dejar campos opcionales vacíos', '', '• Borrar movimientos de stock', '', '• Códigos únicos sin espacios', '', '', ''],
    ['• Agregar nuevas categorías', '', '• Editar la hoja 🔧 Datos', '', '• Revisa 📊 Control Lotes semanalmente', '', '', ''],
    ['• Agregar nuevos proveedores', '', '• Dejar SKU o Nombre vacíos', '', '• Haz backup periódicamente', '', '', ''],
    ['• Agregar nuevas marcas', '', '• Usar códigos duplicados', '', '• Comparte con permisos de "Editor"', '', '', ''],
    ['', '', '', '', '', '', '', ''],
...
export async function addSampleData(spreadsheetId: string = SPREADSHEET_ID): Promise<void> {
  ...
  for (let i = 2; i <= maxCatRows; i++) {
    levelFormulas.push([`=IF(A${i}<>"",IF(B${i}="",1,IF(COUNTIF($B$2:$B$${maxCatRows},B${i})>0,VLOOKUP(B${i},$A$2:$C$${maxCatRows},3,FALSE)+1,2)),"")`]);
  }
  ...
  for (let i = 2; i <= maxCatRows; i++) {
    catCategoryCountFormulas.push([
      `=IF(D${i}<>"",COUNTIF('🆕 Productos'!$C$2:$C$2100,D${i}),"")`
    ]);
  }
  ...
  if (categoryRows.length > 0) {
    categoryBatchData.push(
      { range: `'📂 Categorías'!A2:B${categoryRows.length + 1}`, values: categoryRows.map(r => [r[0], r[1]]) },
      { range: `'📂 Categorías'!D2:F${categoryRows.length + 1}`, values: categoryRows.map(r => [r[2], r[3], r[4]]) },
      { range: `'📂 Categorías'!J2:J${categoryRows.length + 1}`, values: categoryRows.map(r => [r[5]]) }
    );
  }
  ...
  for (let i = 2; i <= maxMisProductosRows; i++) {
    codigoFormulas.push([`=IF(A${i}<>"",IFERROR(INDEX('🆕 Productos'!$A$2:$A$2100,MATCH(A${i},'🆕 Productos'!$B$2:$B$2100,0)),""),"")`]);
  }

  // G: Descripción = INDEX/MATCH from catalog (col N)
  const descripcionFormulas: string[][] = [];
  for (let i = 2; i <= maxMisProductosRows; i++) {
    descripcionFormulas.push([`=IF(A${i}<>"",IFERROR(INDEX('🆕 Productos'!$N$2:$N$2100,MATCH(A${i},'🆕 Productos'!$B$2:$B$2100,0)),""),"")`]);
  }

  // H: Categoría = INDEX/MATCH from catalog (col C)
  const categoriaFormulas: string[][] = [];
  for (let i = 2; i <= maxMisProductosRows; i++) {
    categoriaFormulas.push([`=IF(A${i}<>"",IFERROR(INDEX('🆕 Productos'!$C$2:$C$2100,MATCH(A${i},'🆕 Productos'!$B$2:$B$2100,0)),""),"")`]);
  }

  // I: Marca = INDEX/MATCH from catalog (col D)
  const marcaFormulas: string[][] = [];
  for (let i = 2; i <= maxMisProductosRows; i++) {
    marcaFormulas.push([`=IF(A${i}<>"",IFERROR(INDEX('🆕 Productos'!$D$2:$D$2100,MATCH(A${i},'🆕 Productos'!$B$2:$B$2100,0)),""),"")`]);
  }

  // J: Proveedor = INDEX/MATCH from catalog (col J)
  const proveedorFormulas: string[][] = [];
  for (let i = 2; i <= maxMisProductosRows; i++) {
    proveedorFormulas.push([`=IF(A${i}<>"",IFERROR(INDEX('🆕 Productos'!$J$2:$J$2100,MATCH(A${i},'🆕 Productos'!$B$2:$B$2100,0)),""),"")`]);
  }
  ...
  for (let i = 2; i <= maxMisProductosRows; i++) {
    recetaFormulas.push([`=IF(A${i}<>"",IFERROR(INDEX('🆕 Productos'!$L$2:$L$2100,MATCH(A${i},'🆕 Productos'!$B$2:$B$2100,0)),""),"")`]);
  }
  ...
  for (let i = 2; i <= maxMisProductosRows; i++) {
    ...
    lastCostFormulas.push([`=IF(A${i}<>"",IFERROR(INDEX('📦 Movimientos Stock'!$H$2:$H$1001,MATCH(2,1/('📦 Movimientos Stock'!$B$2:$B$1001=A${i})*1/('📦 Movimientos Stock'!$C$2:$C$1001="Compra")*1/('📦 Movimientos Stock'!$H$2:$H$1001<>""),1)),""),"")`]);
  }
  ...
  for (let i = 2; i <= maxMisProductosRows; i++) {
    stockActualFormulas.push([`=IF(A${i}<>"",SUMPRODUCT(('📦 Movimientos Stock'!$B$2:$B$1001=A${i})*('📦 Movimientos Stock'!$D$2:$D$1001)*('📦 Movimientos Stock'!$N$2:$N$1001)),"")`]);
  }
  ...
  for (let i = 2; i <= maxMisProductosRows; i++) {
    proxVenceFormulas.push([`=IF(A${i}<>"",IFERROR(MINIFS('📦 Movimientos Stock'!$I$2:$I$1001,'📦 Movimientos Stock'!$B$2:$B$1001,A${i},'📦 Movimientos Stock'!$I$2:$I$1001,">"&TODAY()),""),"")`]);
  }

  // S: Alertas = warnings for expiration, low stock, etc.
  const alertasFormulas: string[][] = [];
  for (let i = 2; i <= maxMisProductosRows; i++) {
    alertasFormulas.push([`=IF(A${i}<>"",IFS(O${i}<=0,"❌ Sin stock",AND(R${i}<>"",R${i}<TODAY()),"⚠️ Vencido",AND(R${i}<>"",R${i}<TODAY()+30),"⏰ Por vencer",O${i}<=C${i},"📦 Stock bajo",TRUE,"✅ OK"),"")`]);
  }
  ...
  const findBySku = (sku: string, minStock: number, location: string): [string, number, number, string] | null => {
  ...
  const clinicInventory = clinicInventoryItems.filter((item): item is [string, number, number, string] => item !== null);
  ...
  for (let i = 2; i <= maxStockRows; i++) {
    codigoMovFormulas.push([`=IF(B${i}<>"",IFERROR(VLOOKUP(B${i},'📋 Mis Productos'!$A$2:$F$501,6,FALSE),""),"")`]);
  }
  ...
  const stockMovements = [
    ...
    // ─────────────────────────────────────────────────────────────────────────
    // [Fecha, Producto, Operación, Cantidad, Lote, Ubicación, Responsable, CostoUnit, Vencimiento, Documento]
    ['01/12/2024', 'Royal Canin Medium Adult 15kg', 'Compra', 10, 'L2024-RC01', 'Exhibición', 'Juan Pérez', 537000, '15/12/2025', 'FAC-001-2024'],
    ['01/12/2024', 'Royal Canin Medium Adult 3kg', 'Compra', 15, 'L2024-RC02', 'Exhibición', 'Juan Pérez', 147000, '15/12/2025', 'FAC-001-2024'],
    ['01/12/2024', 'Pro Plan Adult Medium 15kg', 'Compra', 10, 'L2024-PP01', 'Exhibición', 'Juan Pérez', 417000, '10/11/2025', 'FAC-002-2024'],
    ['01/12/2024', 'Dog Chow Adultos Medianos y Grandes 21kg', 'Compra', 15, 'L2024-DC01', 'Depósito Principal', 'Juan Pérez', 255000, '30/10/2025', 'FAC-003-2024'],
    ['01/12/2024', 'Pedigree Adulto 21kg', 'Compra', 18, 'L2024-PD01', 'Depósito Principal', 'Juan Pérez', 195000, '20/09/2025', 'FAC-003-2024'],
    ['01/12/2024', 'Whiskas Adulto Pescado 10kg', 'Compra', 12, 'L2024-WH01', 'Depósito Principal', 'Juan Pérez', 207000, '20/09/2025', 'FAC-004-2024'],

    // Antiparasitarios
    ['01/12/2024', 'Frontline Plus Perros 10-20kg 1 pipeta', 'Compra', 20, 'FL-2024-M01', 'Farmacia', 'Dra. María López', 37200, '30/06/2026', 'FAC-005-2024'],
    ['01/12/2024', 'Frontline Plus Perros 20-40kg 1 pipeta', 'Compra', 15, 'FL-2024-L01', 'Farmacia', 'Dra. María López', 43200, '30/06/2026', 'FAC-005-2024'],
    ['01/12/2024', 'NexGard Perros 10-25kg 1 comprimido', 'Compra', 15, 'NX-2024-M01', 'Farmacia', 'Dra. María López', 69000, '31/08/2026', 'FAC-006-2024'],
    ['01/12/2024', 'Bravecto Perros 20-40kg 1 comprimido', 'Compra', 8, 'BV-2024-L01', 'Farmacia', 'Dra. María López', 237000, '30/09/2026', 'FAC-006-2024'],
    ['01/12/2024', 'Bravecto Perros 10-20kg 1 comprimido', 'Compra', 6, 'BV-2024-M01', 'Farmacia', 'Dra. María López', 207000, '30/09/2026', 'FAC-006-2024'],

    // Vacunas
    ['01/12/2024', 'Nobivac DHPPi 1 dosis', 'Compra', 30, 'NB-DHPP-2024', 'Farmacia', 'Dra. María López', 51000, '30/06/2025', 'FAC-007-2024'],
    ['01/12/2024', 'Nobivac Rabies 1 dosis', 'Compra', 50, 'NB-RAB-2024', 'Farmacia', 'Dra. María López', 27000, '31/12/2025', 'FAC-007-2024'],
    ['01/12/2024', 'Nobivac Tricat Trio 1 dosis', 'Compra', 25, 'NB-TRI-2024', 'Farmacia', 'Dra. María López', 57000, '30/06/2025', 'FAC-007-2024'],
    ['01/12/2024', 'Nobivac Lepto 1 dosis', 'Compra', 20, 'NB-LEP-2024', 'Farmacia', 'Dra. María López', 33000, '30/06/2025', 'FAC-007-2024'],

    // Consumibles y Fluidoterapia
    ['01/12/2024', 'Jeringa BD 3ml con Aguja Caja x 100 (23G)', 'Compra', 6, 'BD-J3-2024', 'Consultorio', 'Juan Pérez', 111000, '31/12/2027', 'FAC-008-2024'],
    ['01/12/2024', 'Jeringa BD 5ml con Aguja Caja x 100 (21G)', 'Compra', 4, 'BD-J5-2024', 'Consultorio', 'Juan Pérez', 135000, '31/12/2027', 'FAC-008-2024'],
    ['01/12/2024', 'Solución Salina 0.9% 500ml', 'Compra', 40, 'SS-2024-01', 'Farmacia', 'Juan Pérez', 15000, '30/11/2025', 'FAC-009-2024'],
    ['01/12/2024', 'Ringer Lactato 500ml', 'Compra', 30, 'RL-2024-01', 'Farmacia', 'Juan Pérez', 16800, '30/11/2025', 'FAC-009-2024'],
    ['01/12/2024', 'Dextrosa 5% 500ml', 'Compra', 20, 'DX-2024-01', 'Farmacia', 'Juan Pérez', 13200, '30/11/2025', 'FAC-009-2024'],

    // Higiene y Accesorios
    ['01/12/2024', 'Catron Arena Natural 10kg', 'Compra', 15, 'CAT-2024-01', 'Depósito Principal', 'Juan Pérez', 69000, '31/12/2026', 'FAC-010-2024'],
    ['01/12/2024', 'Kong Classic M', 'Compra', 8, 'KONG-M-2024', 'Exhibición', 'Juan Pérez', 66300, '', 'FAC-011-2024'],
    ['01/12/2024', 'Ferplast Atlas 20 Transportadora 58x37x32cm', 'Compra', 4, 'FPL-20-2024', 'Exhibición', 'Juan Pérez', 135000, '', 'FAC-011-2024'],

    // ─────────────────────────────────────────────────────────────────────────
    // 📅 02-08/12/2024 - PRIMERA SEMANA DE OPERACIONES
    // ─────────────────────────────────────────────────────────────────────────
    ['02/12/2024', 'Royal Canin Medium Adult 15kg', 'Venta', 2, '', 'Exhibición', 'Ana García', '', '', 'TKT-001-2024'],
    ['03/12/2024', 'Dog Chow Adultos Medianos y Grandes 21kg', 'Venta', 3, '', 'Depósito Principal', 'Ana García', '', '', 'TKT-002-2024'],
    ['04/12/2024', 'Pedigree Adulto 21kg', 'Venta', 4, '', 'Depósito Principal', 'Ana García', '', '', 'TKT-003-2024'],
    ['04/12/2024', 'Frontline Plus Perros 10-20kg 1 pipeta', 'Venta', 3, '', 'Farmacia', 'Dra. María López', '', '', 'TKT-003-2024'],
    ['05/12/2024', 'NexGard Perros 10-25kg 1 comprimido', 'Venta', 2, '', 'Farmacia', 'Dra. María López', '', '', 'TKT-004-2024'],
    ['06/12/2024', 'Whiskas Adulto Pescado 10kg', 'Venta', 2, '', 'Depósito Principal', 'Ana García', '', '', 'TKT-005-2024'],

    // Uso clínico (vacunaciones)
    ['03/12/2024', 'Nobivac DHPPi 1 dosis', 'Uso Interno', 5, '', 'Farmacia', 'Dra. María López', '', '', ''],
    ['04/12/2024', 'Nobivac Rabies 1 dosis', 'Uso Interno', 8, '', 'Farmacia', 'Dra. María López', '', '', ''],
    ['05/12/2024', 'Nobivac Tricat Trio 1 dosis', 'Uso Interno', 3, '', 'Farmacia', 'Dra. María López', '', '', ''],

    // Uso clínico (consultas y cirugías)
    ['06/12/2024', 'Jeringa BD 3ml con Aguja Caja x 100 (23G)', 'Uso Interno', 1, '', 'Consultorio', 'Dra. María López', '', '', ''],
    ['07/12/2024', 'Solución Salina 0.9% 500ml', 'Uso Interno', 6, '', 'Farmacia', 'Dra. María López', '', '', ''],
    ['07/12/2024', 'Ringer Lactato 500ml', 'Uso Interno', 4, '', 'Farmacia', 'Dra. María López', '', '', ''],

    // ─────────────────────────────────────────────────────────────────────────
    // 📅 09-15/12/2024 - SEGUNDA SEMANA
    // ─────────────────────────────────────────────────────────────────────────
    ['09/12/2024', 'Royal Canin Medium Adult 15kg', 'Venta', 3, '', 'Exhibición', 'Ana García', '', '', 'TKT-006-2024'],
    ['10/12/2024', 'Royal Canin Medium Adult 3kg', 'Venta', 2, '', 'Exhibición', 'Ana García', '', '', 'TKT-007-2024'],
    ['10/12/2024', 'Pro Plan Adult Medium 15kg', 'Venta', 2, '', 'Exhibición', 'Ana García', '', '', 'TKT-007-2024'],
    ['11/12/2024', 'Frontline Plus Perros 20-40kg 1 pipeta', 'Venta', 4, '', 'Farmacia', 'Dra. María López', '', '', 'TKT-008-2024'],
    ['11/12/2024', 'Bravecto Perros 20-40kg 1 comprimido', 'Venta', 2, '', 'Farmacia', 'Dra. María López', '', '', 'TKT-008-2024'],
    ['12/12/2024', 'Frontline Plus Gatos 1 pipeta', 'Venta', 3, '', 'Farmacia', 'Dra. María López', '', '', 'TKT-009-2024'],

    // Reposición por bajo stock
    ['12/12/2024', 'Frontline Plus Perros 10-20kg 1 pipeta', 'Compra', 15, 'FL-2024-M02', 'Farmacia', 'Juan Pérez', 37200, '30/06/2026', 'FAC-012-2024'],
    ['13/12/2024', 'Dog Chow Adultos Medianos y Grandes 21kg', 'Compra', 10, 'L2024-DC02', 'Depósito Principal', 'Juan Pérez', 255000, '28/02/2026', 'FAC-013-2024'],
    ['13/12/2024', 'Frontline Plus Gatos 1 pipeta', 'Compra', 12, 'FL-CAT-2024', 'Farmacia', 'Juan Pérez', 33000, '30/06/2026', 'FAC-014-2024'],

    // Más uso clínico
    ['13/12/2024', 'Nobivac DHPPi 1 dosis', 'Uso Interno', 6, '', 'Farmacia', 'Dra. María López', '', '', ''],
    ['13/12/2024', 'Nobivac Rabies 1 dosis', 'Uso Interno', 5, '', 'Farmacia', 'Dra. María López', '', '', ''],
    ['14/12/2024', 'Jeringa BD 3ml con Aguja Caja x 100 (23G)', 'Uso Interno', 1, '', 'Consultorio', 'Dra. María López', '', '', ''],

    // ─────────────────────────────────────────────────────────────────────────
    // 📅 16-22/12/2024 - TERCERA SEMANA (Navidad, más ventas)
    // ─────────────────────────────────────────────────────────────────────────
    ['16/12/2024', 'Royal Canin Medium Adult 15kg', 'Venta', 2, '', 'Exhibición', 'Ana García', '', '', 'TKT-010-2024'],
    ['17/12/2024', 'Royal Canin Medium Adult 3kg', 'Venta', 3, '', 'Exhibición', 'Ana García', '', '', 'TKT-011-2024'],
    ['18/12/2024', 'Kong Classic M', 'Venta', 2, '', 'Exhibición', 'Ana García', '', '', 'TKT-012-2024'],
    ['18/12/2024', 'Ferplast Atlas 20 Transportadora 58x37x32cm', 'Venta', 1, '', 'Exhibición', 'Ana García', '', '', 'TKT-012-2024'],

    // Más ventas y uso
    ['20/12/2024', 'NexGard Perros 10-25kg 1 comprimido', 'Venta', 3, '', 'Farmacia', 'Dra. María López', '', '', 'TKT-013-2024'],
    ['20/12/2024', 'Nobivac DHPPi 1 dosis', 'Uso Interno', 4, '', 'Farmacia', 'Dra. María López', '', '', ''],
    ['21/12/2024', 'Solución Salina 0.9% 500ml', 'Uso Interno', 5, '', 'Farmacia', 'Dra. María López', '', '', ''],

    // ─────────────────────────────────────────────────────────────────────────
    // 📅 23-31/12/2024 - SEMANA NAVIDAD/AÑO NUEVO
    // ─────────────────────────────────────────────────────────────────────────
    ['23/12/2024', 'Pedigree Adulto 21kg', 'Venta', 3, '', 'Depósito Principal', 'Ana García', '', '', 'TKT-014-2024'],
    ['23/12/2024', 'Cat Chow Adultos Carne y Pollo 8kg', 'Venta', 2, '', 'Depósito Principal', 'Ana García', '', '', 'TKT-014-2024'],

    // Producto dañado
    ['26/12/2024', 'Catron Arena Natural 10kg', 'Daño', 1, '', 'Depósito Principal', 'Juan Pérez', '', '', 'INF-001-2024'],

    // Ventas post-navidad
    ['28/12/2024', 'Frontline Plus Perros 10-20kg 1 pipeta', 'Venta', 5, '', 'Farmacia', 'Dra. María López', '', '', 'TKT-015-2024'],
    ['28/12/2024', 'Frontline Plus Perros 20-40kg 1 pipeta', 'Venta', 3, '', 'Farmacia', 'Dra. María López', '', '', 'TKT-015-2024'],
    ['29/12/2024', 'Bravecto Perros 10-20kg 1 comprimido', 'Venta', 2, '', 'Farmacia', 'Dra. María López', '', '', 'TKT-016-2024'],

    // Devolución de cliente
    ['30/12/2024', 'Royal Canin Sterilised 4kg', 'Devolución Cliente', 1, '', 'Exhibición', 'Ana García', '', '', 'DEV-001-2024'],

    // Compra fin de año
    ['30/12/2024', 'Nobivac DHPPi 1 dosis', 'Compra', 25, 'NB-DHPP-2025', 'Farmacia', 'Juan Pérez', 51000, '30/06/2025', 'FAC-015-2024'],
    ['30/12/2024', 'Nobivac Rabies 1 dosis', 'Compra', 30, 'NB-RAB-2025', 'Farmacia', 'Juan Pérez', 27000, '31/12/2025', 'FAC-015-2024'],

    // Último día
    ['31/12/2024', 'Solución Salina 0.9% 500ml', 'Uso Interno', 4, '', 'Farmacia', 'Dr. Carlos Ruiz', '', '', ''],
    ['31/12/2024', 'Ringer Lactato 500ml', 'Uso Interno', 3, '', 'Farmacia', 'Dr. Carlos Ruiz', '', '', ''],
    ['31/12/2024', 'Dextrosa 5% 500ml', 'Uso Interno', 2, '', 'Farmacia', 'Dr. Carlos Ruiz', '', '', ''],
  ];

  // Write data to columns A-J (10 client entry columns)
  // [Fecha, Producto, Operación, Cantidad, Lote, Ubicación, Responsable, CostoUnit, Vencimiento, Documento]
  const stockRows = stockMovements.map(m => [m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7], m[8], m[9]]);
  ...
  for (let i = 2; i <= maxLotesRows; i++) {
    loteCodigoFormulas.push([`=IF(A${i}<>"",IFERROR(VLOOKUP(A${i},'📋 Mis Productos'!$A$2:$F$501,6,FALSE),""),"")`]);
  }

  // D: 📅 F.Ingreso = First purchase date for this Product+Lote
  const loteIngresoFormulas: string[][] = [];
  for (let i = 2; i <= maxLotesRows; i++) {
    loteIngresoFormulas.push([`=IF(AND(A${i}<>"",B${i}<>""),IFERROR(MINIFS('📦 Movimientos Stock'!$A$2:$A$${maxMovRows},'📦 Movimientos Stock'!$B$2:$B$${maxMovRows},A${i},'📦 Movimientos Stock'!$E$2:$E$${maxMovRows},B${i},'📦 Movimientos Stock'!$C$2:$C$${maxMovRows},"Compra"),""),"")`]);
  }
  ...
  for (let i = 2; i <= maxLotesRows; i++) {
    loteVencimientoFormulas.push([`=IF(AND(A${i}<>"",B${i}<>""),IFERROR(INDEX('📦 Movimientos Stock'!$I$2:$I$${maxMovRows},MATCH(1,('📦 Movimientos Stock'!$B$2:$B$${maxMovRows}=A${i})*('📦 Movimientos Stock'!$E$2:$E$${maxMovRows}=B${i})*('📦 Movimientos Stock'!$C$2:$C$${maxMovRows}="Compra"),0)),""),"")`]);
  }
  ...
  for (let i = 2; i <= maxLotesRows; i++) {
    loteCantidadFormulas.push([`=IF(AND(A${i}<>"",B${i}<>""),SUMPRODUCT(('📦 Movimientos Stock'!$B$2:$B$${maxMovRows}=A${i})*('📦 Movimientos Stock'!$E$2:$E$${maxMovRows}=B${i})*('📦 Movimientos Stock'!$D$2:$D$${maxMovRows})*('📦 Movimientos Stock'!$N$2:$N$${maxMovRows})),"")`]);
  }

  // G: 💰 Costo Unit. = Purchase cost for this lot (H is CostoUnit in new structure)
  const loteCostoFormulas: string[][] = [];
  for (let i = 2; i <= maxLotesRows; i++) {
    loteCostoFormulas.push([`=IF(AND(A${i}<>"",B${i}<>""),IFERROR(INDEX('📦 Movimientos Stock'!$H$2:$H$${maxMovRows},MATCH(1,('📦 Movimientos Stock'!$B$2:$B$${maxMovRows}=A${i})*('📦 Movimientos Stock'!$E$2:$E$${maxMovRows}=B${i})*('📦 Movimientos Stock'!$C$2:$C$${maxMovRows}="Compra"),0)),""),"")`]);
  }

  // H: 💵 Valor = Cantidad × Costo Unit.
  const loteValorFormulas: string[][] = [];
  for (let i = 2; i <= maxLotesRows; i++) {
    loteValorFormulas.push([`=IF(AND(F${i}<>"",G${i}<>"",ISNUMBER(F${i}),ISNUMBER(G${i})),F${i}*G${i},"")`]);
  }
  ...
  for (let i = 2; i <= maxLotesRows; i++) {
    loteDiasFormulas.push([`=IF(E${i}<>"",IFERROR(IF(ISNUMBER(E${i}),E${i}-TODAY(),DATEVALUE(E${i})-TODAY()),""),"")`]);
  }

  // J: 🚦 Estado = Simplified status with emoji
  const loteEstadoFormulas: string[][] = [];
  for (let i = 2; i <= maxLotesRows; i++) {
    loteEstadoFormulas.push([`=IF(A${i}<>"",IF(NOT(ISNUMBER(F${i})),"⚪",IF(F${i}<=0,"🔴",IF(NOT(ISNUMBER(I${i})),"🟢",IF(I${i}<0,"⚫",IF(I${i}<=30,"🟡",IF(I${i}<=90,"🟠","🟢")))))),"")`]);
  }
  ...
  for (let i = 2; i <= maxLotesRows; i++) {
    loteFIFOFormulas.push([`=IF(AND(A${i}<>"",ISNUMBER(F${i}),F${i}>0),IFERROR(RANK(IF(ISNUMBER(E${i}),E${i},DATE(2099,12,31))+D${i}/1000000,IF(($F$2:$F$${maxLotesRows}>0)*($A$2:$A$${maxLotesRows}=A${i}),IF(ISNUMBER($E$2:$E$${maxLotesRows}),$E$2:$E$${maxLotesRows},DATE(2099,12,31))+$D$2:$D$${maxLotesRows}/1000000,""),1),""),"")`]);
  }
