...
serve(async (req) => {
  ...
  try {
    ...
    if (!smsBody) {
      return new Response(
        JSON.stringify({ error: 'Missing SMS body or valid template' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (!accountSid || !authToken || !fromNumber) {
      return new Response(
        JSON.stringify({ error: 'SMS not configured for this tenant' }),
    ...
    if (!response.ok) {
      ...
      return new Response(
        JSON.stringify({ error: `SMS failed: ${error.message}` }),
        { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    return new Response(
      JSON.stringify({ success: true, message_id: result.sid }),
